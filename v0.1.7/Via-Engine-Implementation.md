...
- [x] **1. Substrate Drill Void (The Cutout)**
  ...
  - *Engine Implementation (v0.1.7):*
    - **Manifold Punch-Through**: The `add_substrate` function performs analytical cylinder subtractions from the substrate layers.
    - **Net-Aware Structural Carving**: Vias now always drill through conductive material (Copper). If the copper belongs to the same net, it is carved at exactly the drill diameter (0nm clearance) to ensure physical truth while maintaining connectivity.
  - *Post-Realization Pass (v0.1.7):* 
    - A final drill pass ensures that manual routes placed after the initial via placement are correctly carved.
  - *Testing Goal:* Ensure that when the FR4 is rendered, you can see clean cylindrical holes passing through all layers, including overlapping traces.
...
- [x] **2. Antipad (Plane Clearance)**
  ...
  - *Engine Implementation (v0.1.7):*
    - **Net-Aware Filtering**: Copper pours subtract via cylinders only if they belong to different nets. 
    - **Clearance Logic**: Different-net pours use `drill_diameter + 2 * clearance_requirement`. Same-net pours use `drill_diameter` (0nm clearance).
...
- [x] **5. Non-Functional Pad (NFP) Removal**
...
## Via Escape Routing Validation (v0.1.7)

The port escape system (`exit:`/`enter:` syntax) has been validated against all via geometries through the following test cases:

### Test Files

| Test | Via Geometry | Escape Direction | Status |
|------|-------------|-----------------|--------|
| `test_pad_to_vias_escape.hw` | Small PTH vias (`contact(Copper)`, 0.3mm drill) | N/S/E/W from Rect pad | ✅ 8 routes pass |
| `test_ring_to_vias_escape.hw` | Small PTH vias (`contact(Copper)`, 0.3mm drill) | N/S/E/W from Ring pour | ✅ 4 routes pass |
| `test_big_via_to_vias_escape.hw` | Large PTH via (0.6mm drill, 0.4mm ring) as routing pad | N/S/E/W escape to 4 target vias | ✅ 4 routes pass (SDF multi-pin) |

### Key Validation Points

- **Annular ring sufficient copper collar**: The `contact(Copper)` via provides sufficient copper around the drill hole for the escape trace to clip to the correct pad edge.
- **Spatial pour bbox resolution**: Pin anchors co-located with `contact(Copper)` vias resolve their bounding box via `get_pour_bbox_at_position()` spatial query, not by name lookup.
- **Multi-pin via nets**: A large via (routing pad) connecting to multiple smaller vias correctly uses `exit:` specs in SDF pathfinding mode (5-pin net via A*).
